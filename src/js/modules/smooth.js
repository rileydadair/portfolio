...
export default class Smooth {
  ...
  constructor(opt = {}) {
    ...
    this.dom = {
      ...
      scrollbar: this.vars.native || this.options.noscrollbar ? null : {
        ...
        el: create({ selector: 'div', styles: `scrollbar` }),
        drag: {
          el: create({ selector: 'div', styles: 'scrollbar__progress' }),
          delta: 0,
          height: 0
        }
      }
  ...
  run() {
    ...
    if(!this.vars.native && !this.options.noscrollbar) {
      ...
      const value = (Math.abs(this.vars.current) / (Math.round(this.vars.bounding) / bounds))
      let percent = (value / bounds) * 100 - 100;
      this.dom.scrollbar.drag.el.style[this.prefix] = this.getTransformPercent(percent.toFixed(3))
    }
  ...
  getTransformPercent(value) {
    return this.vars.direction === 'vertical' ? `translate3d(0%,${value}%,0)` : `translate3d(${value}%,0%,0)`
  }
